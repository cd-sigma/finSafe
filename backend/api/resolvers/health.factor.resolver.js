...
async function calculateHealthFactor(web3, collateralAssets, collateralAmounts, collateralPrices, debtAssets, debtAmounts, debtPrices, liquidationThresholds) {
    try {
        ...
        for (const cAsset of collateralAssets) {
            const price = collateralPrices[cAsset]
            const amount = collateralAmounts[cAsset]
            const liquidationThreshold = liquidationThresholds[cAsset]
            const priceInEth = priceLib.convertUsdAmountToEth(price * amount, web3)
        }
